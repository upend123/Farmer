...
fun prevMemberPayments(){
    MemberPaymentsScreen()
}

@Composable
fun MemberPaymentsScreen(onBackClick:() -> Unit = {},
                         onAddNewMemberPayment: () -> Unit = {}){
    Column(
        modifier = Modifier
            .fillMaxSize()
            .padding(
        ...
        HeaderWithoutManuIcon(
            title = "Member Payments", isFirstTrailingIcon = true, morningDropDown = false,
            isDateAndMorningBar = true, secondTrailingIcon = R.drawable.baseline_more_vert_24,
            onBackClick = { onBackClick() }, onFirstTrailingIconClick = { onAddNewMemberPayment() }
        )
